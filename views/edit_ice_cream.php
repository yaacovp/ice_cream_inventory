...
<form method="POST" action="controllers/update_ice_cream.php">
    ...
    <select name="flavor_id" required>
        <?php foreach ($flavors as $flavor): ?>
            <option value="<?php echo $flavor['id']; ?>" <?php echo ($ice_cream['flavor_id'] == $flavor['id']) ? 'selected' : ''; ?>>
    ...
    <select name="size" required>
        <option value="500ml" <?php echo ($ice_cream['size'] == '500ml') ? 'selected' : ''; ?>>500ml</option>
        <option value="1L" <?php echo ($ice_cream['size'] == '1L') ? 'selected' : ''; ?>>1L</option>
    </select>
    
    <label>Quantité (nombre de pots) :</label>
    <input type="number" name="stock" min="1" value="<?php echo $ice_cream['stock']; ?>" required>
